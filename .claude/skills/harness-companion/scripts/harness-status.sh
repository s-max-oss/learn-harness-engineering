#!/bin/bash
# harness-status.sh — Harness health dashboard (v2 rewrite)
#
...
#
# Changes from v1:
#   - Replaced `set -e` with `set +e` so missing AGENTS.md doesn't kill the
#     Scope section. `check_file` returns 1 on failure and the script survives.
#   - ASCII markers ([OK]/[NO]/[--]/[!!]) for portability across Git Bash.
#   - Now also checks for .harness/config.json under Environment.
...
if [ -f "feature_list.json" ]; then
  if command -v jq >/dev/null 2>&1; then
    if jq . feature_list.json >/dev/null 2>&1; then
      ...
      # Passing / unverified without evidence
      NO_EVIDENCE="$(jq '[.features[] | select((.status=="passing" or .status=="unverified") and (.evidence | length) == 0)] | length' feature_list.json 2>/dev/null || echo 0)"
      if [ "$NO_EVIDENCE" -gt 0 ]; then
        echo "  [!!] $NO_EVIDENCE passing/unverified features have NO evidence"
        WARNINGS=$((WARNINGS + 1))
      fi

      # Features with override (audit bypass)
      OVERRIDES="$(jq '[.features[] | select(.override != null and (.evidence | length) == 0)] | length' feature_list.json 2>/dev/null || echo 0)"
      if [ "$OVERRIDES" -gt 0 ]; then
        echo "  [!!] $OVERRIDES features overridden to unverified without evidence"
        WARNINGS=$((WARNINGS + 1))
      fi
    else
